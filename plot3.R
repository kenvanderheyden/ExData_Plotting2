...
# generate the plot
plot3 <- ggplot(emisByTypePerYear, aes(x=year, y=emis, group=type, color=type)) + 
        geom_line() + 
        geom_point( size=4, shape=21, fill="white") +
        ggtitle("Plot 3 - Baltimore City: total PM25 emissions by type per year") +
        ylab("emission")
...
ggsave(filename = "plot3.png", plot = plot3, dpi = 100)